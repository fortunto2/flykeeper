...
@Test func walkingMovesAlongTheHeading() {
    let next = motion.advance(origin, command: .full, behaviour: .walk, dt: 0.1)
...
@Test func turningRotatesWithoutLeavingTheSpot() {
    let next = motion.advance(origin, command: .full, behaviour: .turn, dt: 0.1)
...
func sleepingAndRestingChangeNothing(_ b: Behaviour) {
    #expect(motion.advance(origin, command: .full, behaviour: b, dt: 0.5) == origin)
}

@Test func groomingMovesLegsButNotTheBody() {
    let next = motion.advance(origin, command: .full, behaviour: .groom, dt: 0.1)
...
@Test func startleIsAJumpFasterThanAWalkAndBeatsTheWings() {
    let walk = motion.advance(origin, command: .full, behaviour: .walk, dt: 0.1)
    let jump = motion.advance(origin, command: .full, behaviour: .startle, dt: 0.1)
...
@Test func wingsStopBeatingAfterTheStartle() {
    var pose = motion.advance(origin, command: .full, behaviour: .startle, dt: 0.1)
    for _ in 0..<20 { pose = motion.advance(pose, command: .full, behaviour: .walk, dt: 0.1) }
...
func theFlyNeverLeavesTheArena(heading: Double) {
    ...
    for _ in 0..<2000 {
        pose = motion.advance(pose, command: .full, behaviour: .walk, dt: 0.05)
        #expect((0...1).contains(pose.x) && (0...1).contains(pose.y), "escaped: \(pose)")
    }
}

@Test func aStartleTakesOffAndTheFlyGlidesBackToTheFloor() {
    var pose = motion.advance(origin, command: .full, behaviour: .startle, dt: 0.2)
    ...
    let mid = motion.advance(pose, command: .full, behaviour: .rest, dt: 0.1)
    #expect(mid.isAirborne && mid.wingBeat == 1)
    #expect(mid.x != pose.x || mid.y != pose.y, "it flies forward while gliding")
    for _ in 0..<200 { pose = motion.advance(pose, command: .full, behaviour: .rest, dt: 0.05) }
...
@Test func theFlyNeverLeavesTheCube() {
    ...
    for i in 0..<400 {
        pose = motion.advance(pose, command: .full, behaviour: i % 40 == 0 ? .startle : .walk, dt: 0.05)
